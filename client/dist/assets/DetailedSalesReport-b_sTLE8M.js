import{b as Ce,r as a,a as ae,j as e,U as de,b4 as T,aB as Se,G as ke,X as U,Z as Le,a$ as ze,g as Fe,k as Pe,y as De,b8 as Te,b9 as Ee,K as Me,q as d,s as Ae}from"./vendor-Dp7SZ6lv.js";import{u as Re}from"./index-CRUdEL4u.js";import"./scanner-vendor-DfxRpMWJ.js";import"./pdf-vendor-DjsH35t9.js";const le="/api";function ce(){const o=new Date,b=o.getFullYear(),f=String(o.getMonth()+1).padStart(2,"0"),R=String(o.getDate()).padStart(2,"0");return`${b}-${f}-${R}`}const n=o=>`C$${Number(o||0).toFixed(2)}`,$=o=>o?new Date(o).toLocaleString("es-NI",{timeZone:"America/Managua",day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",hour12:!0}):"—",xe=o=>o?new Date(o).toLocaleDateString("es-NI",{timeZone:"America/Managua",day:"2-digit",month:"2-digit",year:"numeric"}):"—",r={primary:"#0f172a",secondary:"#475569",success:"#16a34a",info:"#0284c7",bg:"#f8fafc",border:"#e2e8f0",text:"#1e293b",textLight:"#64748b"},A=Ae`from { opacity: 0; transform: translateY(8px); } to { opacity: 1; transform: translateY(0); }`,Be=d.div`
  max-width: 1500px;
  margin: 0 auto;
  padding: 2rem;
  background: ${r.bg};
...
`,Ge=()=>{const[o,b]=a.useState(window.innerWidth<768);return a.useEffect(()=>{const f=()=>b(window.innerWidth<768);return window.addEventListener("resize",f),()=>window.removeEventListener("resize",f)},[]),o},Ye=o=>{switch(o){case"COMPLETADA":return{background:"#dcfce7",color:"#166534"};case"DEVOLUCION":return{background:"#fee2e2",color:"#991b1b"};case"CANCELADA":return{background:"#fef3c7",color:"#92400e"};default:return{background:"#f1f5f9",color:"#475569"}}},M=({type:o,children:b})=>{const f={fontSize:"0.7rem",fontWeight:700,padding:"3px 8px",borderRadius:"6px",textTransform:"uppercase",letterSpacing:"0.03em",whiteSpace:"nowrap",...Ye(o)};return e.jsx("span",{style:f,children:b})};function Qe(){var re,se,oe,ie,ne;const{token:o,products:b,clients:f}=Re(),R=Ce(),v=Ge(),[i,B]=a.useState("ventas"),[N,me]=a.useState(ce()),[C,ue]=a.useState(ce()),[w,K]=a.useState([]),[Z,X]=a.useState(!1),[ge,be]=a.useState({}),[L,fe]=a.useState(""),[h,je]=a.useState(""),[m,z]=a.useState(null),[ye,F]=a.useState(!1),[J,P]=a.useState(""),[l,I]=a.useState(null),[ve,Q]=a.useState(!1),V=a.useMemo(()=>{const t={"Content-Type":"application/json"};return o&&(t.Authorization=`Bearer ${o}`),t},[o]),y=a.useCallback(async(t,s,c)=>{X(!0);try{const x={startDate:N,endDate:C};t&&(x.tipo=t),s&&(x.keyword=s),c&&(x.clientId=c);const p=await ae.get(`${le}/reports/detailed-sales`,{headers:V,params:x});K(Array.isArray(p.data)?p.data:[])}catch(x){console.error("Error fetching detailed sales:",x),K([])}finally{X(!1)}},[V,N,C]);a.useEffect(()=>{i==="ventas"?y(null,null,m==null?void 0:m.id_cliente):i==="devoluciones"?y("DEVOLUCION"):i==="busqueda"&&h.trim().length>=3&&y(null,h)},[i,N,C,y,h,m]);const S=a.useMemo(()=>{if(!L)return b;const t=L.toLowerCase();return b.filter(s=>s.nombre.toLowerCase().includes(t)||String(s.codigo).toLowerCase().includes(t))},[b,L]),ee=async t=>{if(t){Q(!0);try{const s=await ae.get(`${le}/reports/product-history`,{headers:V,params:{code:t.codigo}});I({product:t,history:Array.isArray(s.data.history)?s.data.history:Array.isArray(s.data)?s.data:[]})}catch(s){console.error("Error fetching product history:",s),I({product:t,history:[]})}finally{Q(!1)}}},we=t=>{be(s=>({...s,[t]:!s[t]}))},q=w.reduce((t,s)=>t+Number(s.totalVenta||0),0),D=w.length,te=t=>{if(!t)return"—";const s=[];return t.efectivo>0&&s.push(`Efectivo: ${n(t.efectivo)}`),t.tarjeta>0&&s.push(`Tarjeta: ${n(t.tarjeta)}`),t.transferencia>0&&s.push(`Transf: ${n(t.transferencia)}`),t.credito>0&&s.push(`Crédito: ${n(t.credito)}`),t.dolares>0&&s.push(`USD: $${Number(t.dolares).toFixed(2)}`),s.length?s.join(" | "):"Efectivo"},$e=()=>{var k;const t=window.open("","_blank");if(!t)return;const s=i==="producto",c=s?`Historial de Producto: ${((k=l==null?void 0:l.product)==null?void 0:k.nombre)||""}`:`Reporte de ${i==="devoluciones"?"Devoluciones":"Ventas Detalladas"}`;s||`${xe(N)}${xe(C)}`;let x="";const p=`
